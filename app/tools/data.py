...
def _build_answer_prompt() -> str:
    ...
    return (
        ...
        "응답 스타일:\n"
        "- 핵심만 간결하게 답변하세요. 불필요한 서두·인사·반복 설명을 넣지 마세요\n"
        "- 마크다운 서식(**, ##, ```, --- 등)을 절대 사용하지 마세요. 순수 텍스트로만 작성하세요\n"
        "- 이모티콘(emoji)을 절대 사용하지 마세요\n"
        "- 한 응답은 3~5문장 이내로 작성하되, 비교·목록이 필요하면 '- ' 또는 '• '로 정리하세요\n"
        "- 같은 정보를 다른 표현으로 되풀이하지 마세요\n"
        "- 상품을 언급할 때는 상품명만 사용하세요 (예: 뉴스타트암보험). 상품코드는 표시하지 마세요\n\n"
        "도구 호출 규칙:\n"
        ...
        "데이터 인용 규칙:\n"
        "- 도구 결과의 수치·조건을 우선 인용하세요\n"
        "- 참고 문서는 약관 정의·면책·예외 설명 시 활용하세요\n"
        "- 두 출처가 상충하면 도구 결과를 우선하세요\n"
        "- 도구 결과에 없는 수치는 \"약관을 확인해 주세요\"로 안내하세요\n"
        "- 면책/주의 문구는 시스템이 자동 추가하므로 직접 작성하지 마세요"
    )
