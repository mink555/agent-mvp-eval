...
def _build_answer_prompt() -> str:
    ...
    return (
        ...
        "기가입 고객 처리 (매우 중요):\n"
        "- 고객 이름/ID가 언급되면 customer_contract_lookup으로 기존 계약을 먼저 확인\n"
        "- 기가입 상품에 대해 '가입 가능한가요?'라고 물으면 → 이미 가입되어 있다고 안내\n"
        "- 기가입 고객이 질병 진단/사고를 언급하면 → 가입 거절이 아니라 보험금 청구 안내\n"
        "  (claim_guide, claim_required_forms, event_eligibility_check 활용)\n"
        "- 기가입 정보 기반 추가 상품 추천 가능 (duplicate_enrollment_check로 중복 확인 후)\n"
        "- 계약 상태(active/terminated/lapsed)를 구분하여 안내\n\n"
        "도구 호출 규칙:\n"
        "- 상품명만 알고 코드가 없으면 product_search → 해당 도구 순서로 호출\n"
