...
_CARDS: list[ToolCard] = [
    ...
    ToolCard(
        ...
        when_not_to_use=(
            ...
            "계약자 목표에 맞는 특약을 추천받고 싶다 → rider_bundle_recommend 사용",
        ),
        tags=("특약", "목록"),
    ),
    ...
    ToolCard(
        ...
        when_not_to_use=(
            "특정 계약자의 병력·나이로 실제 가입 가능 여부를 판단해야 한다 → underwriting_precheck 사용",
        ),
        tags=("가입조건", "자격확인"),
    ),
    ...
    ToolCard(
        ...
        purpose="계약자의 목표(암 대비·치아 보호 등)에 맞는 특약 조합을 추천한다. 어떤 특약을 붙이면 좋은지 추천이 필요할 때 사용한다.",
        when_to_use=(
            "암 대비에 좋은 특약 추천해줘",
            "치아 관련 특약 조합",
            "어떤 특약 붙이는 게 좋아?",
    ...
    ToolCard(
        ...
        purpose="나이·성별·병력 요약을 기반으로 계약자가 보험에 가입 가능한지 인수 적합성을 사전 판단한다. 계약자의 병력·건강 이력으로 가입 가능 여부를 확인할 때 사용한다.",
        when_to_use=(
            "당뇨 이력 있어도 가입 가능해?",
            "고혈압인데 암보험 들 수 있어?",
            "병력 있는 계약자 인수 가능 여부 확인",
            "55세 남성 기존 수술 이력 있는데 가입돼?",
            "기존 질환 있는데 보험 가입 될까?",
            "이 계약자 병력으로 인수 심사 통과 가능해?",
        ),
        when_not_to_use=(
    ...
    ToolCard(
        ...
        when_not_to_use=(
            "특정 계약자 병력으로 가입 가능한지 확인하고 싶다 → underwriting_precheck 사용",
        ),
        tags=("인수심사", "거절조건", "녹아웃"),
    ),
    ...
    ToolCard(
        ...
        purpose="계약자 ID 또는 이름으로 기존 계약 목록을 조회한다.",
        when_to_use=(
            "홍길동 계약 조회",
            "계약자 ID C001의 계약 내역",
            "이 계약자 어떤 보험 들었어?",
    ...
    ToolCard(
        ...
        purpose="계약자의 기존 계약을 기반으로 특정 상품 추가 가입 가능 여부를 판단한다.",
        when_to_use=(
            "이 계약자 암보험 중복 가입돼?",
            "이미 가입한 상품이랑 중복 여부",
            "동일 상품 또 들 수 있어?",
    ...
    ToolCard(
        ...
        purpose="이름·나이·성별 조건으로 계약자를 검색한다.",
        when_to_use=(
            "40대 남성 계약자 찾아줘",
            "홍씨 성의 계약자 목록",
            "계약자 검색",
